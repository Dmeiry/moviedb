...
class LazyPackageRegistry {
    ...
    async scanPackages() {
        ...
        const names = stripped.map((s) => lodash_1.last(s.split(`${path_1.default.sep}node_modules${path_1.default.sep}`)));
...
async function findFiles(rootDir, filename, list = []) {
    const dirList = await fs_extra_1.default.readdir(rootDir);
    for (const f of dirList) {
        const full = path_1.default.join(rootDir, f);
