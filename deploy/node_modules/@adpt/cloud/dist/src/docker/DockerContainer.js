...
exports.containerStatus = exports.DockerContainer = exports.computeContainerName = void 0;
...
async function networkDiff(info, props, op) {
    async function resolver(names) {
        const infos = await cli_1.dockerInspect(names, Object.assign(Object.assign({}, props), { type: "network" }));
...
function mountsUpToDate(info, _context, props) {
    ...
    const expected = expectedIn.map((m) => (Object.assign(Object.assign({}, mountDefaultProps), m)));
...
function privilegedUpToDate(info, _context, props) {
    const ctr = info.data;
    if (!ctr)
        throw new utils_1.InternalError(`No container report`);
...
async function containerIsUpToDate(info, context, props) {
    ...
    if (!mountsUpToDate(info, context, props))
        return "replace";
    if (!privilegedUpToDate(info, context, props))
        return "replace";
    /*
...
async function updateContainer(info, _context, props) {
    ...
    await cli_1.dockerNetworkConnect(info.name, diff.toAdd, Object.assign(Object.assign({}, props), { alreadyConnectedError: false }));
    await cli_1.dockerNetworkDisconnect(info.name, diff.toDelete, Object.assign(Object.assign({}, props), { alreadyDisconnectedError: false }));
}
...
async function runContainer(context, props) {
    ...
    const opts = Object.assign(Object.assign({}, propsNoNetworks), { name,
        image, labels: Object.assign(Object.assign({}, (props.labels || {})), { [labels_1.adaptDockerDeployIDKey]: `${context.buildData.deployID}` }), network: (networks && networks[0]) || undefined });
...
class DockerContainer extends action_1.Action {
    ...
    async refreshContainerInfo(context, force = false) {
        if (!this.info || force) {
...
DockerContainer.defaultProps = {
    dockerHost: cli_1.defaultDockerHost,
    privileged: false,
};
