...
exports.dockerPull = exports.dockerPush = exports.dockerNetworkDisconnect = exports.dockerNetworkConnect = exports.dockerRun = exports.dockerRm = exports.dockerStop = exports.dockerNetworks = exports.dockerNetworkLs = exports.dockerInspect = exports.createTag = exports.dockerRemoveImage = exports.dockerTag = exports.dockerImageId = exports.dockerBuild = exports.defaultDockerBuildOptions = exports.execDocker = exports.withFilesImage = exports.buildFilesImage = exports.writeFiles = exports.busyboxImage = exports.defaultDockerHost = exports.pickGlobals = exports.exec = exports.debugOut = exports.debug = void 0;
...
exports.debug = debug_1.default("adapt:cloud:docker");
// Enable with DEBUG=adapt:cloud:docker:out*
exports.debugOut = debug_1.default("adapt:cloud:docker:out");
exports.exec = utils_1.debugExec(exports.debug, exports.debugOut);
exports.pickGlobals = (opts) => lodash_1.default.pick(opts, "dockerHost");
exports.defaultDockerHost = process.env.DOCKER_HOST ||
    os_1.default.platform() === "win32" ? "npipe:////./pipe/docker_engine" : "unix:///var/run/docker.sock";
/**
...
async function buildFilesImage(files, opts) {
    ...
    return utils_1.withTmpDir(async (dir) => {
        ...
        return dockerBuild("-", dir, Object.assign(Object.assign({}, exports.pickGlobals(opts)), { forceRm: true, imageName: "adapt-tmp-files", uniqueTag: true, stdin: dockerfile, deployID: opts.deployID }));
...
const buildKitUnsupported = new Map();
/** @internal */
async function execDocker(argsIn, options) {
    const globalArgs = [];
    ...
    try {
        return await exports.exec("docker", args, execaOpts);
    }
    catch (e) {
        if (e.all) {
...
async function dockerBuild(dockerfile, contextPath, options = {}) {
    const opts = Object.assign(Object.assign({}, exports.defaultDockerBuildOptions), options);
    const mRef = image_ref_1.mutableImageRef({
    ...
    if (opts.imageName) {
        ...
        if (tag)
            mRef.tag = tag;
        else if (!opts.uniqueTag && !opts.imageTag)
            mRef.tag = "latest";
        if (!opts.uniqueTag && mRef.nameTag)
            args.push("-t", mRef.nameTag);
    }
    if (opts.deployID) {
    ...
    if (opts.uniqueTag) {
        ...
        if (prevId === id)
            mRef.nameTag = opts.prevUniqueNameTag; // prev points to current id
        else {
            if (!mRef.nameTag)
                throw new utils_1.InternalError(`nameTag not set`);
            await dockerTag(Object.assign({ existing: id, newTag: mRef.nameTag }, exports.pickGlobals(opts)));
        }
    }
    const ret = mRef.freeze();
    if (!image_ref_1.isImageRefDockerhostWithId(ret)) {
        throw new utils_1.InternalError(`Built image reference '${mRef.ref}' is not ` +
            `a valid dockerhost reference with ID`);
    }
    return ret;
}
exports.dockerBuild = dockerBuild;
async function idFromBuild(stdout, stderr, opts) {
...
async function dockerRemoveImage(options) {
    const opts = Object.assign(Object.assign({}, dockerRemoveImageDefaults), options);
    const args = ["rmi"];
    if (opts.force)
        args.push("--force");
...
async function dockerNetworks(opts) {
    ...
    return dockerInspect(networks, Object.assign(Object.assign({}, opts), { type: "network" }));
}
...
async function dockerRun(options) {
    const opts = Object.assign(Object.assign({}, defaultDockerRunOptions), options);
...
async function dockerPull(opts) {
    ...
    const info = await dockerInspect([registryDigest], Object.assign({ type: "image" }, exports.pickGlobals(opts)));
    if (info.length !== 1) {
