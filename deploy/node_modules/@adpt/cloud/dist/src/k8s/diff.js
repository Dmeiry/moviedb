...
function listFiles(dir, list = [], stripDir = true) {
    const dirList = fs_1.default.readdirSync(dir);
    dirList.forEach((f) => {
        const full = stripDir ? f : path_1.default.join(dir, f);
        if (fileType(f) === "DIR") {
...
function diffFiles(path1, path2, header = false) {
    const file1 = fileOrEmpty(path1);
    const file2 = fileOrEmpty(path2);
    const diff = diff_1.diffLines(file1, file2);
    diff.forEach((part) => {
...
function diffDirs(dir1, dir2) {
    const paths = new Set(listFiles(dir1));
    listFiles(dir2).forEach((f) => paths.add(f));
    const sorted = [...paths].sort();
    sorted.forEach((p) => {
        const f1 = path_1.default.join(dir1, p);
        const f2 = path_1.default.join(dir2, p);
