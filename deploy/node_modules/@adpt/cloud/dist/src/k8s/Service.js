...
exports.serviceResourceInfo = exports.Service = exports.k8sServiceProps = void 0;
    ...
    updateSelector(async () => {
        ...
        if (epProps.kind === "Pod") {
            return utils_1.removeUndef({
                [manifest_support_1.labelKey("name")]: manifest_support_1.resourceElementToName(ep.target, deployID)
            });
        }
        if (hasSelector(epProps)) {
            const sel = epProps.spec.selector;
            if (common_1.isLabelSelector(sel)) {
...
function hasSelector(props) {
    return props.spec.selector != undefined;
}
function makeSvcManifest(props, options) {
    const { config, key, handle, metadata } = props, spec = tslib_1.__rest(props, ["config", "key", "handle", "metadata"]);
    ...
    return {
        ...
        spec: Object.assign(Object.assign({}, spec), { selector: core_1.isHandle(spec.selector) ? options.endpointSelector : spec.selector }),
        config,
    };
}
