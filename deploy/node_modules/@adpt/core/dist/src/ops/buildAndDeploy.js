...
exports.buildAndDeploy = exports.deployPass = exports.withContext = exports.observe = exports.build = exports.currentState = exports.computePaths = void 0;
...
async function currentState(options) {
    ...
    const prevComplete = await deployment.lastEntry(history_1.HistoryStatus.complete);
    const prevTry = await deployment.lastEntry();
    const observationsJson = (_b = (_a = options.observationsJson) !== null && _a !== void 0 ? _a : prevComplete === null || prevComplete === void 0 ? void 0 : prevComplete.observationsJson) !== null && _b !== void 0 ? _b : "{}";
    const prevStateJson = (_d = (_c = options.prevStateJson) !== null && _c !== void 0 ? _c : prevComplete === null || prevComplete === void 0 ? void 0 : prevComplete.stateJson) !== null && _d !== void 0 ? _d : "{}";
    const prevDependenciesJson = (_e = prevComplete === null || prevComplete === void 0 ? void 0 : prevComplete.dependenciesJson) !== null && _e !== void 0 ? _e : "{}";
    let stateStore;
    try {
    ...
    const ret = Object.assign(Object.assign(Object.assign({}, options), paths), { commit, ignoreDeleteErrors: Boolean(options.ignoreDeleteErrors), observationsJson,
        prevDependenciesJson, prevDomXml: prevComplete === null || prevComplete === void 0 ? void 0 : prevComplete.domXml, prevStateJson,
        deployOpID,
        stackName,
    ...
    async function commit(entry) {
        ...
        if (!ret.dryRun) {
            await deployment.commitEntry(Object.assign(Object.assign({}, entry), { fileName: ret.fileName, projectRoot: ret.projectRoot, stackName: ret.stackName, stateJson: stateStore.serialize() }));
        }
    }
}
...
async function build(options) {
    return withContext(options, async (ctx) => {
        ...
        let needsData = {};
        const { root, style } = await stack_1.resolveStack(stackName, ctx);
        if (root != null) {
        ...
        return Object.assign(Object.assign({}, options), { mountedElements,
...
async function observe(options) {
    ...
    const ret = withContext(options, async (ctx) => {
        ...
        return Object.assign(Object.assign({}, orig), { observationsJson: serialize_1.stringifyFullObservations({
...
async function reanimateAndBuild(opts) {
    ...
    if (domXml === undefined)
        return { dom: null, mountedElements: [] };
    let stateStore;
    try {
...
async function deployPass(options) {
    const { actTaskObserver, dataDir, deployment, prevDependencies, prevDom, prevMountedElements, taskObserver } = options, buildOpts = tslib_1.__rest(options, ["actTaskObserver", "dataDir", "deployment", "prevDependencies", "prevDom", "prevMountedElements", "taskObserver"]);
    return withContext(options, async (ctx) => {
        // This is the inner context's copy of Adapt
        const inAdapt = ctx.Adapt;
        debugAction(`deployPass: rebuild`);
        taskObserver.updateStatus("Rebuilding DOM");
        const buildResults = await build(Object.assign(Object.assign({}, buildOpts), { deployment, withStatus: true, taskObserver }));
        ...
        try {
            ...
            return Object.assign(Object.assign({}, buildResults), { dependenciesJson,
                deployComplete,
                stateChanged,
                observationsJson });
        }
...
async function buildAndDeploy(options) {
    ...
    return withContext(initial, async (ctx) => {
        ...
        try {
            ...
            const build1 = await tasks.build.complete(() => build(Object.assign(Object.assign({}, initial), { ctx, withStatus: false, taskObserver: tasks.build })));
            debugAction(`buildAndDeploy: reanimate`);
            const prev = await tasks.reanimatePrev.complete(async () => reanimateAndBuild({
                ctx,
            ...
            const prevDependencies = JSON.parse(initial.prevDependenciesJson);
            debugAction(`buildAndDeploy: observe`);
            const observeOptions = Object.assign(Object.assign({}, build1), { taskObserver: tasks.observe });
            ...
            const result = await tasks.deploy.complete(() => deployTasks.status.complete(async () => {
                try {
                    ...
                    const passOpts = Object.assign(Object.assign({}, fromBuild), { actTaskObserver: deployTasks.act, dataDir,
                        prevDependencies, prevDom: prev.dom, prevMountedElements: prev.mountedElements, taskObserver: deployTasks.status });
                    while (true) {
