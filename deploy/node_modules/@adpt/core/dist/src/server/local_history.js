...
exports.createLocalHistoryStore = exports.infoFilename = exports.observationsFilename = exports.stateFilename = exports.domFilename = exports.dependenciesFilename = exports.dataDirName = void 0;
...
exports.dependenciesFilename = "adapt_dependencies.json";
...
class LocalHistoryStore {
    ...
    async commitEntry(toStore) {
        const { dependenciesJson, domXml, stateJson, observationsJson } = toStore, info = tslib_1.__rest(toStore, ["dependenciesJson", "domXml", "stateJson", "observationsJson"]);
        ...
        await fs.outputFile(path.join(dirPath, exports.dependenciesFilename), dependenciesJson);
    ...
    async historyEntry(historyName) {
        ...
        try {
            dependenciesJson = (await fs.readFile(path.join(dirName, exports.dependenciesFilename))).toString();
        }
        catch (err) {
            if (err.code !== "ENOENT")
                throw err;
    ...
    async getDataDir(withStatus) {
        ...
        try {
            this.dataDirRelease = await lockfile_1.lock(this.dataDir, "Adapt server history lock");
        }
        catch (e) {
            throw new utils_1.UserError(`Unable to get exclusive access to deployment ` +
    ...
    async nextDirName(status) {
        ...
        const timestamp = moment_1.default().format().replace(/:/g, ".");
