...
exports.listDeployments = exports.listDeploymentIDs = exports.destroyDeployment = exports.loadDeployment = exports.createDeployment = exports.encodePathComponent = void 0;
...
async function loadDeployment(server, deployID) {
    try {
        ...
        if (!isPathNotFound(err))
            throw err;
        throw new utils_1.UserError(`Deployment '${deployID}' does not exist`);
    }
    const deployment = new DeploymentImpl(deployID, server);
    await deployment.init();
    return deployment;
...
class DeploymentImpl {
    ...
    async status(stepID, info) {
        ...
        if (info !== undefined) {
            ...
            return server_1.withLock(this.server, async (lock) => {
                ...
                await this.server.set(path, Object.assign(Object.assign({}, cur), info), { lock });
                return;
            });
        }
        // Get
    ...
    async elementStatus(stepID, idOrMap) {
        ...
        return server_1.withLock(this.server, async (lock) => {
            ...
            await this.server.set(path, Object.assign(Object.assign({}, old), idOrMap), { lock });
        });
    }
    async newOpID() {
        return server_1.withLock(this.server, async (lock) => {
