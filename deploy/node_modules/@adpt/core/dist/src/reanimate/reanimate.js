...
exports.reanimateUrn = exports.findMummyUrn = exports.mockRegistry_ = exports.findMummy = exports.reanimate = exports.callerModule = exports.findConstructorModule = exports.registerConstructor = exports.registerObject = exports.MummyRegistry = void 0;
...
class MummyRegistry {
    constructor(projectRoot) {
        ...
        try {
            this.packageRegistry = utils_1.createPackageRegistry(projectRoot);
        }
        catch (err) {
...
function registry() {
    if (!_registry)
        throw new error_1.InternalError(`Reanimate registry not initialized`);
    return _registry;
}
function initRegistry() {
    ...
    let projectRoot;
    try {
        projectRoot = global.getAdaptContext().projectRoot;
    }
    catch (err) {
...
function enbalm(obj, name, namespace, module) {
    ...
    const m = {
        ...
        relFilePath: utils_1.posixPath(path.relative(path.dirname(pkgInfo.main), module.filename)),
    };
...
function registerObject(obj, name, modOrCallerNum = 0, altNamespace = "$adaptExports") {
    if (!_registry)
        return; // Only register objects when we're in a project
    if (obj == null)
