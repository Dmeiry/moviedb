...
exports.createPluginConfig = exports.registerPlugin = exports.checkPrimitiveActions = exports.isPluginManagerImpl = exports.createPluginManager = void 0;
...
class PluginManagerImpl {
    ...
    async start(opts) {
        ...
        const options = Object.assign(Object.assign({}, defaultStartOptions), opts);
        this.deployment_ = options.deployment;
        this.deployOpID_ = options.deployOpID;
        this.goalStatus_ = options.newDom === null ? deploy_types_1.GoalStatus.Destroyed : deploy_types_1.GoalStatus.Deployed;
    ...
    analyze() {
        ...
        if (dom && !jsx_1.isFinalDomElement(dom)) {
            throw new error_1.InternalError(`dom is not MountedPrimitive`);
        }
        if (prevDom && !jsx_1.isMountedElement(prevDom)) {
            throw new error_1.InternalError(`prevDom is not Mounted`);
        }
        const diff = dom_utils_1.domDiff(prevDom, dom);
        checkPrimitiveActions(diff, this.actions);
        this.plan_ = execution_plan_1.createExecutionPlan({
            actions: this.actions,
            dependencies: this.prevDependencies,
            deployment: this.deployment,
            deployOpID: this.deployOpID,
            diff: dom_utils_1.domDiffElements(this.prevMountedElements, this.newMountedElements),
            newDom: dom,
    ...
    async act(options) {
        const opts = Object.assign(Object.assign({}, defaultActOptions), options);
        ...
        const { deploymentStatus, stateChanged } = await execution_plan_1.execute(Object.assign(Object.assign({}, opts), { logger,
            plan }));
    ...
    get deployment() {
        if (this.deployment_ === undefined)
            throw new error_1.InternalError(`Must call start before accessing deployment`);
        return this.deployment_;
    }
    get deployOpID() {
        if (this.deployOpID_ === undefined)
            throw new error_1.InternalError(`Must call start before accessing deployOpID`);
        return this.deployOpID_;
    }
    get goalStatus() {
        if (this.goalStatus_ === undefined)
            throw new error_1.InternalError(`Must call start before accessing goalStatus`);
        return this.goalStatus_;
    }
    get logger() {
        if (this.logger_ === undefined) {
            throw new error_1.InternalError(`Must call start before accessing logger`);
        }
        return this.logger_;
    }
    get newDom() {
        if (this.newDom_ === undefined) {
            throw new error_1.InternalError(`Must call start before accessing newDom`);
        }
        return this.newDom_;
    }
    get newMountedElements() {
        if (this.newMountedElements_ === undefined) {
            throw new error_1.InternalError(`Must call start before accessing newMountedElements`);
        }
        return this.newMountedElements_;
    }
    get plan() {
        if (this.plan_ === undefined)
            throw new error_1.InternalError(`Must call analyze before accessing plan`);
        return this.plan_;
    }
    get prevDependencies() {
        if (this.prevDependencies_ === undefined) {
            throw new error_1.InternalError(`Must call start before accessing prevDependencies`);
        }
        return this.prevDependencies_;
    }
    get prevDom() {
        if (this.prevDom_ === undefined) {
            throw new error_1.InternalError(`Must call start before accessing prevDom`);
        }
        return this.prevDom_;
    }
    get prevMountedElements() {
        if (this.prevMountedElements_ === undefined) {
            throw new error_1.InternalError(`Must call start before accessing prevMountedElements`);
        }
...
function registerPlugin(plugin) {
    ...
    const mod = Object.assign(Object.assign({}, plugin), { packageName: pInfo.name, version: pInfo.version });
